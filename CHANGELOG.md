...
- 新增 `docs/EXAMPLES_1_3_VOLTAGE_VARIANT_COMPARISON.md`：逐檔比較 5 V、12 V、16 V 單環例程，標記設定、PI、REP、啟動安全與 Keil 產物的原始碼位置。
...
- 確認例程 1～3 的 256 個相對路徑檔案中有 249 個逐位元組相同、7 個不同；控制行為差異集中於 `state_machine.c`、`PID.c` 與 `HRTIM.c`。
- 確認輸出版本除目標電壓外，還調整 OVP、最大 duty、例程 3 的 PI/REP；另辨識 `PID_INT()` 未套用初始脈寬與 `DISdriver` 啟動漂移。
...
- 對例程 1～3 的相對路徑聯集執行 SHA-256 比較，並逐行複核全部 7 個不同檔案；未修改任何 C/H 或 Keil 工程檔。
