...
void PID_INT(void)//PID初始化
{
	...
	Pulse_width=0.15f*PWM_PERIOD*0.303030f;
}
/*****************************************************
...
void PID_loop(float voltage,float Current)
{
	...
	if(V_PI.error_add>2.31f)V_PI.error_add=2.31f;//限制最大占空比:2.31f/3.3f=70%
	else if(V_PI.error_add<0.15f)V_PI.error_add=0.15f;//限制最小占空比:0.15f/3.3f=4.5%
	//电流环
	I_PI.error1=Iref-Current;
	I_PI.error_add+=I_PI.Kp_i*I_PI.error1-I_PI.Kp*I_PI.error2;
	I_PI.error2=I_PI.error1;
	if(I_PI.error_add>2.31f)I_PI.error_add=2.31f;//限制最大占空比:2.31f/3.3f=70%
	else if(I_PI.error_add<0.15f)I_PI.error_add=0.15f;//限制最小占空比:0.15f/3.3f=4.5%
  //双环竞争，占空比=(误差电压/3.3V)，脉宽=占空比*周期
	Pulse_width=(V_PI.error_add>I_PI.error_add)?PWM_K*I_PI.error_add:PWM_K*V_PI.error_add;//脉宽计算可以到哔哩哔哩观看我们的视频教程
	//更新寄存器
	HRTIM1->sTimerxRegs[HRTIM_TIMERINDEX_TIMER_A].CMP1xR=Pulse_width;//更新占空比
	HRTIM1->sTimerxRegs[HRTIM_TIMERINDEX_TIMER_A].CMP2xR=Pulse_width>>1;//在脉宽中点采集数据，避免噪声   
}
